...
/**
 * OnboardingReact component for rendering the onboarding steps.
 *
 ...
 *
 * @returns {JSX.Element} The rendered OnboardingReact component.
 *
 * @example
 * <OnboardingReact
 *   steps={steps}
...
const OnboardingReact = ({ children, steps, shadowRgb = '0, 0, 0', shadowOpacity = '0.2', cardTransition = { ease: 'anticipate', duration: 0.6 }, cardComponent: CardComponent, onStart = () => { }, onStepChange = () => { }, onComplete = () => { }, onSkip = () => { }, displayArrow = true, clickThroughOverlay = false, navigationAdapter = useWindowAdapter, disableConsoleLogs = false, scrollToTop = true, noInViewScroll = false, }) => {
    ...
    }, [currentTour, onStart, isOnboardingVisible]);
    // - -
    // Initialize
    useEffect(() => {
    ...
    }, [currentStep, currentPath, currentTourSteps, isOnboardingVisible]);
    // - -
    ...
    useEffect(() => {
        if (!isOnboardingVisible || !currentTour)
            return;
        if (!currentTourSteps || currentStep === undefined)
            return;
        const step = currentTourSteps[currentStep];
    ...
    useEffect(() => {
        const handleKeyDown = (event) => {
            if (isOnboardingVisible && !currentTourSteps?.[currentStep]?.blockKeyboardControl) {
                switch (event.key) {
                    case 'ArrowRight':
                        nextStep();
                        break;
    ...
    }, [isOnboardingVisible, nextStep, prevStep, skipTour]);
    // - -
    ...
    return (_jsxs("div", { "data-name": "nextstep-wrapper", "data-nextstep": "dev", style: { position: 'relative', width: '100%' }, children: [_jsx("div", { "data-name": "nextstep-site", style: { display: 'block', width: '100%' }, children: children }), pointerPosition && isOnboardingVisible && viewport && (_jsx(DynamicPortal, { viewportID: currentTourSteps?.[currentStep]?.viewportID, children: _jsxs(motion.div, { "data-name": "nextstep-overlay", initial: "hidden", animate: isOnboardingVisible ? 'visible' : 'hidden', variants: variants, transition: { duration: 0.5 }, style: {
                        position: 'absolute',
                        top: 0,
                        left: 0,
                        overflow: 'hidden',
...
export default OnboardingReact;
